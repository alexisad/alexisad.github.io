...
proc getDistricts(d: string): TableRef[string, District] =
    ...
    for distr in arrDistricts:
        let
            ...
            numAdminPlaceLayer = if isCity: 8 else: 9
        if isCity:
    ...
    let
        ...
        urlAdminPlaceDistr = fmt"https://s.fleet.ls.hereapi.com/1/index.json?apikey={apikey}&layer=ADMIN_PLACE_9&attributes=ADMIN_PLACE_ID&values={distrValues}"
        resAdminDistrIdx = parseJson(client.getContent urlAdminPlaceDistr)
        cityValues = encodeUrl(cityIds.join ",")
        urlAdminPlaceCity = fmt"https://s.fleet.ls.hereapi.com/1/index.json?apikey={apikey}&layer=ADMIN_PLACE_8&attributes=ADMIN_PLACE_ID&values={cityValues}"
    ...
    for i in 0..dTileXYs.high:
        let
            ...
            urlPolyAdmin = fmt"https://s.fleet.ls.hereapi.com/1/tiles.txt?apikey={apikey}&tilexy={allTileXYs}&layers={layers}&levels={levels}"
...
proc getRoads() =
    ...
    for i in 0..dTileXYs.high:
        let
            ...
            levels = encodeUrl(dLevels[i].join ",")
            layers = encodeUrl(dLayers[i].join ",")
            urlRoad = fmt"https://s.fleet.ls.hereapi.com/1/tiles.txt?apikey={apikey}&meta=1&tilexy={allTileXYs}&layers={layers}&levels={levels}"
        data.add (client.getContent urlRoad)
        echo "i:", i
    var roadsFile: File
    discard roadsFile.open("roads.txt", fmAppend)
