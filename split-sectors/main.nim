...
import jsony, flatty, supersnappy
...
proc getDistricts(d: string): TableRef[string, District] =
    ...
    for i in 0..dTileXYs.high:
        ...
        if i > 0:
            arrResPolyAdmin.delete(0, 0) #delete header
...
template readHead(b: untyped): untyped =
    if willHead:
        ...
        if exceptsLinks.contains linkId:
            ...
            if not tblRoadLinks.hasKeyOrPut(linkId, new RoadLink):
                tblRoadLinks[linkId].linkId = linkId
            #echo "linkId:", linkId, tblRoadLinks.len
            b


proc filterRoadLinks(strm: Stream, tblDistrict: TableRef[string, District]): TableRef[string, RoadLink] =
    var
    ...
    for line in strm.lines():
        ...
        of "ROAD_GEOM":
            ...
            readHead:
                ...
                tblRoadLinks[linkId].coords = parseCoords(arrRow[idxLat], arrRow[idxLon])
        of "ROAD_NAME":
            #echo "ROAD_NAME!!!!"
            readHead:
                ...
                let
                    ...
                    streetNames = parseStreetNames arrRow[iNames]
                tblRoadLinks[linkId].name = streetNames
                #for strName in streetNames:
...
proc main() =
    ...
    when false:
        ...
        var tblRoadLinks = filterRoadLinks(openFileStream "roadLinks.txt", tblDistrict)
        let data = tblRoadLinks.toFlatty().compress()
        let sData = openFileStream("roadLinks.data", fmWrite)
        sData.write(data)
        sData.close
        tblRoadLinks.clear
    when true:
        let sDataR = openFileStream "roadLinks.data"
        let tblRoadLinks = sDataR.readAll().uncompress().fromFlatty(TableRef[string, RoadLink])
        echo tblRoadLinks["52418711"].streetName
        echo tblRoadLinks["1200131157"].streetName
        echo tblRoadLinks["990689333"].streetName
        echo tblRoadLinks["1157468789"].streetName
        #echo tblRoadLinks["52410981"].streetName
