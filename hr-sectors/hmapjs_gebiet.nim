...
import macros, typetraits, algorithm, unicode, uri
...
const visualDistricts = slurp("visual_districts.csv")
                        ...
                        for nStreet, links in pairs(v.streets):
                            ...
                            let totalFam =
                                if tblTotalFam.hasKey(streetObj.hash):
                                    ...
                                    0
                            let
                                coord = links[0].geometry[0]
                                ci = fmt"{v.postalCode}+{v.city}"
                                st = encodeUrl(nStreet, true)
                            tdiv(class = "street"):
                                tdiv:
                                    span("data-lat" = $coord.lat, "data-lng" = $coord.lng, onclick = clckOnStr):
                                        text [nStreet, " (", $totalFam, ")"].join""
                                    span(class = "tel-book"):
                                        a(href = fmt"https://www.dasoertliche.de/?zvo_ok=0&ci={ci}&st={st}&radius=0&form_name=search_nat_ext",
                                                target = "_blank"):
                                            text "das Örtl."
...
proc showBorder(hrCoord: string, sCol: string, latlon = true, lineWidth = 10, allCommas = false) =
    let hrArrArea =
        if allCommas:
            hrCoord.split ","
        else:
...
proc geoArea() =
    ...
    #consLog("vDistr:", visualDistricts.split("\n"))
    when false:
        let arrDistricts = visualDistricts.split("\n")
        for vDistr in arrDistricts:
            #consLog("vDistr:", vDistr)
            showBorder(vDistr, "rgba(0, 0, 255, 1)", lineWidth = 3, allCommas = true)
